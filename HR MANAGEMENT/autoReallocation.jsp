...
<html>
    ...
    ArrayList<Integer> salesfromPL1Previous = new ArrayList<Integer>();
        ...
        ResultSet resultset = st1.executeQuery("SELECT * FROM dailysales WHERE EXTRACT(MONTH FROM date)='"+month+"' AND category_id ='1'");
        while(resultset.next()){
        int sales = resultset.getInt("totalSales");
        salesfromPL1Current.add(sales);
    }
        ResultSet resultset1 = st1.executeQuery("SELECT * FROM dailysales WHERE EXTRACT(MONTH FROM date)='"+PMonth+"' AND category_id ='1'");
...
//         out.println(sumPrevious);
    ...
    ResultSet resultset2 = st1.executeQuery("SELECT * FROM dailysales WHERE EXTRACT(MONTH FROM date)='"+month+"' AND category_id ='2'");
    while(resultset2.next()){
    int sales2 = resultset2.getInt("totalSales");
    salesfromPL2Current.add(sales2);
    
    }
    ResultSet resultset22 = st1.executeQuery("SELECT * FROM dailysales WHERE EXTRACT(MONTH FROM date)='"+PMonth+"' AND category_id ='2'");
...
//    out.println(sumPrevious22);
    ...
    ResultSet resultset3 = st1.executeQuery("SELECT * FROM dailysales WHERE EXTRACT(MONTH FROM date)='"+month+"' AND category_id ='3'");
    while(resultset3.next()){
    int sales3 = resultset3.getInt("totalSales");
    salesfromPL3Current.add(sales3);
    
    }
    ResultSet resultset33 = st1.executeQuery("SELECT * FROM dailysales WHERE EXTRACT(MONTH FROM date)='"+PMonth+"' AND category_id ='3'");
...
//    out.println(sumPrevious33);
    ...
    ResultSet resultset4 = st1.executeQuery("SELECT * FROM dailysales WHERE EXTRACT(MONTH FROM date)='"+month+"' AND category_id ='4'");
    while(resultset4.next()){
    int sales4 = resultset4.getInt("totalSales");
    salesfromPL4Current.add(sales4);
    
    }
    ResultSet resultset44 = st1.executeQuery("SELECT * FROM dailysales WHERE EXTRACT(MONTH FROM date)='"+PMonth+"' AND category_id ='4'");
...
//   out.println("This is the highest priority line:  "+ changeMap.get(max));
   ...
   // swap staff from the lowest categoty priority line to the highest category priority line 
    ResultSet s = st1.executeQuery("SELECT staff_id FROM staff WHERE category_id ='"+lowestPriority+"'");
    ArrayList<Integer> staffIDS_lowestPriority = new ArrayList<Integer>();
              while(s.next()){                
                int staffIds = s.getInt("staff_id");                 
...
//            out.println(staffIDS_lowestPriority); 
            ...
            while(n<staffIDS_lowestPriority.size()){
                        st1.executeUpdate("UPDATE staff SET category_id = '"+highestPriority+"' WHERE staff_id = '"+ staffIDS_lowestPriority.get(n)+"'");           
                        n++;
                    }  
