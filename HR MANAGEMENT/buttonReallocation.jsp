...
//To do, count number of employees in the staff table
              ...
              ResultSet Employees = st1.executeQuery("SELECT COUNT(staff_id) as employees FROM staff ");
...
//     SELECT SUM INSTEAD OF ADDING ___SELECT ONLY WHAT WE WANT
    ...
    ResultSet resultset = st1.executeQuery("SELECT SUM(totalSales) as TS FROM dailysales WHERE EXTRACT(MONTH FROM date)='"+month+"' AND category_id ='1'");
    int sales1 = 0;
...
//     SELECT SUM INSTEAD OF ADDING ___SELECT ONLY WHAT WE WANT
    ...
    ResultSet resultset2 = st1.executeQuery("SELECT SUM(totalSales) as TS2 FROM dailysales WHERE EXTRACT(MONTH FROM date)='"+month+"' AND category_id ='2'");
    int sales2 =0;
...
// PRODUCT CATEGORY LINE 3
    ResultSet resultset3 = st1.executeQuery("SELECT SUM(totalSales) as TS3 FROM dailysales WHERE EXTRACT(MONTH FROM date)='"+month+"' AND category_id ='3'");
    int sales3 = 0;
...
//    FOR PRODUCT LINE 3
    ResultSet resultset4= st1.executeQuery("SELECT SUM(totalSales) as TS4 FROM dailysales WHERE EXTRACT(MONTH FROM date)='"+month+"' AND category_id ='4'");
    int sales4 = 0;
   ...
   int finalEmp4 = Math.round(ApproximateEmp4);
    ...
    //Execute query to return 4 sets of staff IDs using Offset and limit clauses;
        ...
        if(newNumberEmployees==totalEmployees){
              ...
              ResultSet staff_productline1 = st1.executeQuery("SELECT staff_id FROM staff ORDER BY staff_id Limit " +finalEmp1+ " Offset 0");
              
              while(staff_productline1.next()){                
                int staffId_1 = staff_productline1.getInt("staff_id");                 
                staffIDS_productLine1.add(staffId_1);                                     
            } 
           
            
            //Skip the first two and get the one staff ID to be assigned to product line 2
             ResultSet staff_productline2 = st1.executeQuery("SELECT staff_id FROM staff ORDER BY staff_id Limit " +finalEmp2+ " Offset "+ finalEmp1+"");
             
              while(staff_productline2.next()){                
                int staffId_2 = staff_productline2.getInt("staff_id");                 
                staffIDS_productLine2.add(staffId_2);                                     
            } 
                
            
            //Skip the first 3, get next 2 staff IDs for PL3;
            ResultSet staff_productline3 = st1.executeQuery("SELECT staff_id FROM staff ORDER BY staff_id Limit "+finalEmp3+" Offset " +(finalEmp1+finalEmp2)+"");
             
              while(staff_productline3.next()){                
                int staffId_3 = staff_productline3.getInt("staff_id");                 
                staffIDS_productLine3.add(staffId_3);                                     
            } 
            
            
            //Skip the first 5, get next 5 staff IDs for PL4;
            ResultSet staff_productline4 = st1.executeQuery("SELECT staff_id FROM staff ORDER BY staff_id Limit "+finalEmp4+ " Offset "+ (finalEmp1+finalEmp2+finalEmp3)+"");
             
              while(staff_productline4.next()){                
                int staffId_4 = staff_productline4.getInt("staff_id");                 
...
//            
            ...
            while(n_productLine1<staffIDS_productLine1.size()){
                st1.executeUpdate("UPDATE staff SET category_id = '1' WHERE staff_id = '"+ staffIDS_productLine1.get(n_productLine1)+"'");           
                n_productLine1++;
            }
            
            //Assign product line 2 staff
            while(n_productLine2<staffIDS_productLine2.size()){
                st1.executeUpdate("UPDATE staff SET category_id = '2' WHERE staff_id = '"+ staffIDS_productLine2.get(n_productLine2)+"'");           
                n_productLine2++;
            }
            
            //Assign product line 3 staff
            while(n_productLine3<staffIDS_productLine3.size()){
                st1.executeUpdate("UPDATE staff SET category_id = '3' WHERE staff_id = '"+ staffIDS_productLine3.get(n_productLine3)+"'");           
                n_productLine3++;
            }
            
            //Assign product line 4 staff
            while(n_productLine4<staffIDS_productLine4.size()){
                st1.executeUpdate("UPDATE staff SET category_id = '4' WHERE staff_id = '"+ staffIDS_productLine4.get(n_productLine4)+"'");           
                n_productLine4++;
            }  
